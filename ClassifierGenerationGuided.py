...
def return_data(path):
    ...
    #CSV path
def run_main_CTRL_guided(data_augmentation = False, do_unlikelihood = False, classifier_path = "classifier_50t_aug", output_path = "guided_CTRLfinetune", inp_epochs = 10, lr=5e-5):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_i, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            batch_inputs = {'input_ids': batch['input_ids'].to("cuda"), 'attention_mask': batch['attention_mask'].to("cuda")}
            output_strings = [training_data[i]['source'][3:] for i in batch['indices']]
...
if __name__ == "__main__":
    print("-----------------new Run!")
    run_main(data_augmentation = True, classifier_path = "classifier_50t_aug", output_path = "not_used_guided_CTRLfinetune_1")
    print("-----------------new Run!")
    run_main(data_augmentation = True, classifier_path = "classifier_50t_aug", output_path = "guided_CTRLfinetune_2")